...
async function guardarEscrutinioPoceada(resultado, datosControlPrevio, usuario) {
  try {
    ...
    // Guardar premios por agencia (de los ganadores del 1er premio - agenciero)
    if (agenciero?.detalles && agenciero.detalles.length > 0) {
      await guardarPremiosPorAgenciaPoceada(escrutinioId, agenciero);
    }

    // No hay detalle de ganadores individual en Poceada como en Quiniela
    // Los ganadores se acumulan por nivel, no individualmente
